...
async function run() {
  ...
  for (let run = 1; run <= 100; run++) {
    ...
    while (game.state === 'running' && game.elapsed < 180) {
      ...
      for (const b of game.balls) {
        ...
        if (b.rank && b.y >= neonJunction.goalY && !points.finish) points.finish = { seconds: game.elapsed, x: b.x };
      }
    }
    const arrivals = game.arrivals.map(b => ({ position: b.id + 1, ticket: Number(b.name), rank: b.rank, forced: b.y < neonJunction.goalY, pipe: routes.get(b.id), checkpoints: checkpoints.get(b.id) }));
    runs.push({ run, finished: game.state === 'finished', finishReason: game.finishReason ?? 'timeout', rescues: game.rescues, seconds: game.elapsed, shakes, escaped: [...escaped], starts, arrivals,
      unfinished: game.balls.filter(b => !b.rank || b.y < neonJunction.goalY).map(b => ({ position: b.id + 1, x: b.x, y: b.y, pipe: routes.get(b.id) })),
      winners: game.winners.map(b => Number(b.name)) });
    if (run % 10 === 0) console.log(run + '/100 completed');
  }
  physics.clearMarbles(); physics.clear();
  const completedRuns = runs.filter(r => r.finishReason === 'arrived');
  const analyzedTrials = completedRuns.length;
  const stalledTrials = runs.filter(r => r.finishReason === 'stalled').length;
  const timedOutTrials = runs.filter(r => r.finishReason === 'timeout').length;
  ...
  for (const run of completedRuns) for (const a of run.arrivals) {
  ...
  for (const row of [...positions, ...players]) { row.winPercent = analyzedTrials ? row.wins / analyzedTrials * 100 : null; row.meanRank = row.finishes ? row.rankSum / row.finishes : null; }
  ...
  const report = { input, count: names.length, trials: runs.length, analyzedTrials, stalledTrials, timedOutTrials, statisticsBasis: 'normally-completed-races', expectedWinsPerTicket: analyzedTrials * input.picks / names.length,
    expectedFirstsPerTicket: analyzedTrials / names.length, failures: runs.length - analyzedTrials,
    uniqueArrivalOrders: new Set(completedRuns.map(r => r.arrivals.map(a => a.position).join(','))).size,
    totalUnsticking: runs.reduce((n,r) => n + r.shakes, 0),
    totalEscaped: runs.reduce((n, r) => n + r.escaped.length, 0),
    sourceSha256: Object.fromEntries(['src/data/neon-junction.ts', 'src/physics-box2d.ts', 'src/game.ts', 'src/race.ts', 'src/wall-guard.ts'].map(file =>
  ...
  console.log(JSON.stringify({ count: report.count, analyzedTrials, stalledTrials, timedOutTrials, failures: report.failures, uniqueArrivalOrders: report.uniqueArrivalOrders, totalUnsticking: report.totalUnsticking, totalEscaped: report.totalEscaped, positions, players, pipes }));
}
