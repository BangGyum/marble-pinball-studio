...
const { desireJar: stage } = require('../src/data/desire-jar.ts');
...
async function run() {
  ...
  for (let run = 1; run <= 100; run++) {
    ...
    const arrivals = game.arrivals.map(b => ({ position: b.id + 1, ticket: Number(b.name), rank: b.rank, forced: b.y < stage.goalY }));
    runs.push({ remaining: game.balls.filter(b => !b.rank || b.y < stage.goalY).map(b => ({position: b.id + 1, ticket: Number(b.name), x: b.x, y: b.y, stuck: b.stuck})), run, finished: game.state === 'finished', finishReason: game.finishReason ?? 'timeout', rescues: game.rescues, seconds: game.elapsed, shakes, starts, arrivals,
  ...
  const report = { timeLimitSeconds: 600, map: '욕망의 항아리', builtinIndex: stages.indexOf(stage), sourceHashes: Object.fromEntries(['src/game.ts', 'src/physics-box2d.ts', 'src/model.ts', 'src/race.ts', 'src/wall-guard.ts', 'src/data/desire-jar.ts'].map(file => [file, crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')])), input, count: names.length, trials: runs.length, analyzedTrials, stalledTrials, timedOutTrials, statisticsBasis: 'normally-completed-races', expectedWinsPerTicket: analyzedTrials * input.picks / names.length,
  ...
  console.log(JSON.stringify({ count: report.count, analyzedTrials, stalledTrials, timedOutTrials, failures: report.failures, uniqueArrivalOrders: report.uniqueArrivalOrders, totalUnsticking: report.totalUnsticking, positions, players }));
}
